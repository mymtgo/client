...
const props = defineProps<{
    ...
    disabled?: boolean;
    /** Text shown when nothing is selected. Defaults to the overlay's wording. */
    placeholder?: string;
    /** Extra classes for the trigger, e.g. to match a form's SelectTrigger. */
    triggerClass?: string;
}>();
...
/** Regular archetypes first, then fallbacks: the same order the list renders in. */
const navigable = computed(() => [...regular.value, ...fallbacks.value]);

const highlighted = ref(0);
const listEl = ref<HTMLElement | null>(null);

watch([search, navigable], () => {
    highlighted.value = 0;
});

function scrollHighlightedIntoView(): void {
    nextTick(() => {
        listEl.value?.querySelector<HTMLElement>('[data-highlighted="true"]')?.scrollIntoView({ block: 'nearest' });
    });
}
...
function onSearchKeydown(event: KeyboardEvent): void {
    if (event.key === 'ArrowDown') {
        ...
        event.preventDefault();
        const target = navigable.value[highlighted.value];
...
function isHighlighted(archetypeId: number): boolean {
    return navigable.value[highlighted.value]?.id === archetypeId;
}

function highlightIndexOf(archetypeId: number): void {
    const index = navigable.value.findIndex((a) => a.id === archetypeId);
...
<template>
    <Popover v-model:open="open">
        <PopoverTrigger
            ...
            :class="
                cn(
                    'flex w-full items-center justify-between gap-2 rounded-md border border-border bg-background px-2 py-1.5 text-left text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-50',
                    props.triggerClass,
                )
            "
            style="-webkit-app-region: no-drag"
        >
            <span class="truncate" :class="props.currentArchetypeName ? '' : 'text-muted-foreground'">
                {{ props.currentArchetypeName ?? props.placeholder ?? 'Unknown archetype' }}
            </span>
        ...
        <PopoverContent
            ...
            style="-webkit-app-region: no-drag"
        >
            <Input v-model="search" placeholder="Search archetypes…" class="h-7 text-xs" @keydown="onSearchKeydown" />

            <div ref="listEl" class="max-h-56 overflow-y-auto" role="listbox">
                <button
                    v-for="archetype in regular"
                    :key="archetype.id"
                    type="button"
                    role="option"
                    :aria-selected="archetype.id === props.currentArchetypeId"
                    :data-highlighted="isHighlighted(archetype.id)"
                    class="flex w-full items-center justify-between gap-2 rounded px-2 py-1 text-left text-xs hover:bg-accent"
                    :class="isHighlighted(archetype.id) ? 'bg-accent' : ''"
                    @mousemove="highlightIndexOf(archetype.id)"
