                ...
                <Card>
                    ...
                    <CardContent class="flex flex-col gap-3">
                        <div class="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                            <span>Download v{{ download.version }}:</span>
                            <Badge :variant="download.status === 'failed' ? 'destructive' : 'outline'"
                                >{{ download.status }}{{ download.error ? ` (${download.error})` : '' }}</Badge
                            >
                            <span v-if="download.status === 'downloading'">{{ download.bytes }} / {{ download.total ?? '?' }} bytes</span>
                        </div>
                        ...
                        <div v-else class="flex flex-col gap-2 text-sm">
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <Badge :variant="settings.available ? 'default' : 'outline'">{{
                            ...
                            <div class="grid grid-cols-2 gap-x-6 gap-y-1 text-xs text-muted-foreground sm:grid-cols-3">
                                <div>
                                    MTGO version: <span class="text-foreground">{{ status.mtgo_version ?? '-' }}</span>
                                </div>
                                <div>
                                    SDK version: <span class="text-foreground">{{ status.sdk_version ?? '-' }}</span>
                                </div>
                                <div>
                                    Sidecar version: <span class="text-foreground">{{ status.sidecar_version ?? '-' }}</span>
                                </div>
                                <div>
                                    Current file: <span class="text-foreground">{{ status.current_file ?? '-' }}</span>
                                </div>
                                <div>
                                    Heartbeat: <span class="text-foreground">{{ status.heartbeat ?? '-' }}</span>
                                </div>
                                <div>
                                    Last event seq: <span class="text-foreground">{{ status.last_event_seq ?? '-' }}</span>
                                </div>
                            </div>
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    <TableRow v-for="event in recentEvents" :key="event.id">
                                        <TableCell class="text-xs whitespace-nowrap">{{ event.seq }}</TableCell>
                                        <TableCell class="text-xs whitespace-nowrap">{{ event.ts }}</TableCell>
                                        <TableCell class="text-xs whitespace-nowrap">{{ event.type }}</TableCell>
                                        <TableCell class="text-xs whitespace-nowrap">{{ event.game_mtgo_id ?? '-' }}</TableCell>
                                        <TableCell class="text-xs">
                                            <Check v-if="event.verified" class="h-3.5 w-3.5 text-emerald-500" />
                                            <X v-else class="h-3.5 w-3.5 text-red-400" />
                                        </TableCell>
                                        <TableCell
                                            class="max-w-[320px] truncate font-mono text-xs text-muted-foreground"
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    <TableRow v-for="diff in diffs" :key="diff.id">
                                        <TableCell class="text-xs whitespace-nowrap">{{ diff.match?.mtgo_id ?? '-' }}</TableCell>
                                        <TableCell class="text-xs whitespace-nowrap">{{ diff.game?.mtgo_id ?? '-' }}</TableCell>
                                        <TableCell class="text-xs whitespace-nowrap">{{ fieldLabels[diff.field] ?? diff.field }}</TableCell>
                                        <TableCell class="text-xs whitespace-nowrap">{{ diff.chosen_source }}</TableCell>
                                        <TableCell
                                            class="max-w-[240px] truncate font-mono text-xs text-muted-foreground"
                                            :title="JSON.stringify(diff.log_value)"
                                        >
                                            {{ stringifyValue(diff.log_value) }}
                                        </TableCell>
                                        <TableCell
                                            class="max-w-[240px] truncate font-mono text-xs text-muted-foreground"
                                            :title="JSON.stringify(diff.sidecar_value)"
                                        >
