...
type ArchetypeStats = {
    archetype: { id: number; name: string; format: string; colorIdentity: string | null };
    deckCount: number;
...
defineProps<{ data: ArchetypeStats | null; config: WidgetConfig }>();
</script>
        ...
        <CardHeader class="pb-2">
            <div class="flex min-w-0 flex-col gap-1">
                <div class="flex min-w-0 items-center gap-2">
                    <CardTitle class="truncate text-base leading-tight font-semibold">{{ data?.archetype.name ?? 'Archetype' }}</CardTitle>
                    <Badge v-if="data" variant="outline">{{ data.archetype.format }}</Badge>
                </div>
                <span v-if="data" class="text-xs text-muted-foreground"> {{ data.deckCount }} {{ data.deckCount === 1 ? 'deck' : 'decks' }} </span>
            </div>
        </CardHeader>
        <CardContent>
            <div v-if="!data" class="py-6 text-center text-sm text-muted-foreground">This archetype is no longer available.</div>
