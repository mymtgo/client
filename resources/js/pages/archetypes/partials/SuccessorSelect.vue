...
const selected = computed<SuccessorOption | null>(
    ...
    <Popover :open="open" @update:open="onOpenChange">
        ...
        <PopoverContent class="w-72 p-0" align="start">
            ...
            <div class="max-h-64 overflow-y-auto p-1">
                <button
                    type="button"
                    class="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-left text-sm italic text-muted-foreground hover:bg-accent/50"
                    @click="select(null)"
                >
                ...
                <p v-if="filtered.length === 0" class="px-2 py-1.5 text-sm text-muted-foreground">
                    No archetypes found.
                </p>
                ...
                >
                    ...
                    <span
                        v-if="option.format !== format"
                        class="shrink-0 text-xs uppercase text-muted-foreground"
                    >
                        {{ option.format }}
                    </span>
                </button>
