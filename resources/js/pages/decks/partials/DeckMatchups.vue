            ...
            <Table v-else>
                <TableHeader class="bg-muted sticky top-0">
                    <TableRow>
                        ...
                        <TableHead class="cursor-pointer select-none text-right" @click="toggleSort('record')">
                            <span class="inline-flex items-center justify-end gap-1">Record <component :is="sortIcon('record')" class="size-3" /></span>
                        </TableHead>
                        <TableHead class="cursor-pointer select-none text-right" @click="toggleSort('matches')">
                            <span class="inline-flex items-center justify-end gap-1">Matches <component :is="sortIcon('matches')" class="size-3" /></span>
                        </TableHead>
                        <TableHead class="cursor-pointer select-none text-right" @click="toggleSort('winrate')">
                ...
                <TableBody>
                    <TableRow v-for="matchup in sortedMatchups" :key="matchup.archetype_id">
                        ...
                        <TableCell class="text-right tabular-nums text-muted-foreground">{{ matchup.matches }}</TableCell>
