...
import { Heart, BookOpen, Hand, X } from 'lucide-vue-next';
...
<template>
    <div v-if="!event" class="rounded-lg border border-dashed p-8 text-center text-muted-foreground">
        No event selected.
    </div>
    ...
    <div v-else class="flex overflow-hidden rounded-lg border bg-card">
        ...
        <div v-if="hasOpenPanels" class="flex shrink-0 border-r">
            ...
            >
                ...
                <div class="flex-1 overflow-y-auto p-1.5 scrollbar-none">
                    ...
                    <div v-else class="flex h-full items-center justify-center py-4 text-xs text-muted-foreground">
                        Empty
                    </div>
                </div>
            </div>
