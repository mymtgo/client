...
<template>
    ...
    <div v-if="entry.type === 'end'" class="mt-2 flex items-center gap-2 pt-2 border-t">
        <div
            ...
            :class="entry.result === 'W' ? 'bg-win' : 'bg-destructive'"
        />
    ...
    <div v-else class="flex items-start gap-2 py-1">
        <div class="mt-1.5 shrink-0">
            <!-- Mulligan: amber dot -->
            <div v-if="entry.type === 'mulligan'" class="h-2 w-2 rounded-full bg-amber-400" />
            <!-- Play event: muted dot, local vs opponent tinted differently -->
            <div
                v-else
                class="h-2 w-2 rounded-full"
                :class="entry.actor === 'local' ? 'bg-win/60' : 'bg-muted-foreground/40'"
            />
        </div>
