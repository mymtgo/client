...
class ResolveMatchDeckFromLog
{
    /**
     * The deck version the log says the match used, from the first game's
     * deck_used event. Never writes. found=false means it stopped before the
     * lookup (no games, no deck_used event, no cards), which callers must
     * treat as "leave the column alone"; found=true with a null version
     * means the lookup ran and matched nothing.
     *
     * @return array{found: bool, version: ?DeckVersion}
     */
    public static function run(MtgoMatch $match): array
    {
        ...
        return ['found' => true, 'version' => $deckVersion];
    }
}
